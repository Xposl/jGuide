...
var Guide = Guide || {settings:{},extension:{},actions:{}};
(function($){
    Guide.settings = $.extend({
      title: 'Help',
      configs: Actions, //Your Guide Lists
      extension_path: './guide/extensions/',
      extensions: ['moveto','eventclick','message','inputvalue','event','drag'],
    },Guide.settings);
    var settings = Guide.settings;
    var current_guide = null;
    var action_running = Guide.extension ? Guide.extension : {};
    var action_index = 0;
    
    
    var $help_group = $('<div id="kissit-help-group" ></div>');
    ...
    $(document).ready(function(){
      ...
      for(var i=0; i < settings.extensions.length; i++){
        ...
        s.src = settings['extension_path']+'guide.'+extionsion+".js";
        $("head").append(s);
      }
      action_running = Guide.extension ? Guide.extension : {};
    ...
    //description
    Guide.displayMessage = function(description,className,callback){
    ...
    Guide.clearMessages = function(callback){
